...
* Remove trailing semicolons from tf_ros sources (`#187 <https://github.com/ros/geometry/issues/187>`_)
  * [tf_ros] Removed trailing semicolons after functions from all sources
...
* Maintain & expose tf2 Buffer in shared_ptr for tf_ros (`#163 <https://github.com/ros/geometry/issues/163>`_)
  ...
  (otherwise changes to tf_ros source headers are never detected
...
* using TimeStamp and FrameId in message filter
  this allows to use tf_ros::MessageFilter with pcl::PointCloud<T>
...
* add queue_size to tf_ros publisher
  `ros/ros_comm#169 <https://github.com/ros/ros_comm/issues/169>`_
...
1.10.7 (2013-12-27)
-------------------
* fix bug in tf_ros::Matrix3x3::getEulerYPR()
  Fixes a bug in tf_ros::Matrix3x3::getEulerYPR() implementation's handling
  of gimbal lock cases (when the new x axis aligns with the old +/-z
  axis).
* add test that demonstrated bug in tf_ros::Matrix3x3
  tf_ros::Matrix3x3::getEulerYPR() has a bug which returns an incorrect rpy
  for certain corner case inputs.  This test demonstrates that bug.
* Fix const correctness of tf_ros::Vector3 rotate() method
  The method does not modify the class thus should be const.
  This has already been fixed in Bullet itself.
* add automatic tf_ros buffer cleaning on bag loop for python
...
* Update listener.py
  Fix the tf_ros listener service exception in rospy. See:
  http://answers.ros.org/question/10777/service-exception-using-tf_ros-listener-in-rospy/
...
1.10.5 (2013-07-19)
-------------------
* tf_ros: export dependency on tf2_ros
...
1.10.2 (2013-07-09)
-------------------
* strip leading slashes in resolve, and also any time a method is passed from tf_ros to tf2 assert the leading slash is stripped as well.  tf_ros::resolve with two arguments will end up with foo/bar instead of /foo/bar.  Fixes https://github.com/ros/geometry_experimental/issues/12
...
* using CATKIN_ENABLE_TESTING to optionally configure tests in tf_ros
...
* removing unused functions
  ...
  removing cache_time from tf_ros, passing through method to tf2 buffer_core
...
* mostly completed conversion of tf_ros::TransformListener to use tf2 under the hood
* lookuptwist working
* tf_ros::Transformer converted to use tf2::Buffer under the hood.  passing tf_unittest.cpp
* making tf_ros exceptions typedefs of tf2 exceptions for compatability
...
-------------------------
* Adding correct install targets for tf_ros scripts
...
* tf_ros depended on angles but did not find_package it
...
1.9.18 (2012-10-16)
-------------------
* tf_ros: Fixed wrong install directory for python message files.
* tf_ros: fixed bug where generated python message code was not being installed.
* tf_ros: added setup.py file and changed CMakeLists.txt to install python files and bound library (_tf.so, also known as pytf_py in CMakeLists.txt) which must have been missed during the previous catkin-ization.
...
1.9.14 (2012-09-18)
-------------------
* patch from Tom Ruehr from tf_ros sig
...
* tf_ros: MessageFilter: added public getter/setter for ``queue_size_``
* adding btQuaternion constructor for ease of use
* fixing method naming for camelCase and adding bt* Constructor methods
* tf_ros.tfwtf now uses rosgraph.Master instead of roslib
* Added tf_ros and angles to catkin
...
* changing all bt* to tf_ros* in LinearMath to avoid collisions
...
* adding tf_ros namespaces to moved files
...
* moving linear math into tf_ros namespace
...
* Not calling ros::Time::now() in tf_ros.cpp, causes problems with pytf
...
* Josh's optimizations from tf2 merged into tf_ros. Tests pass
...
* removing reset_time topic and catching negative time change to reset the tf_ros buffer
...
* make tf_ros unittest a ros node
...
* tf_ros: change_notifier should sleep after an exception
...
* `#4039 <https://github.com/ros/geometry/issues/4039>`_, moved PoseMath from tf_ros to tf_conversions
...
* tf_ros: change_notifier now supports multiple frames; publishes tfMessages
...
* tf_ros: changed manifest to have lower-case tf_ros
* comment
* more documentation
...
* renaming tf_ros::remap to tf_ros::resolve as per `#3190 <https://github.com/ros/geometry/issues/3190>`_ with backwards compatability.  Also Standardizing to only do searchparam at startup `#3167 <https://github.com/ros/geometry/issues/3167>`_
...
* Add a failure callback to tf_ros::MessageFilter
* fixing `#2990 <https://github.com/ros/geometry/issues/2990>`_ deprecated ~ call
* update tf_ros error strings. Still need review and user testing
* notifier should subscribe to tf_ros and tf_message
...
* tf_ros monitor working, and a little bit cleaner display
...
* Rename tf_ros message from \tf_message to \tf_ros. Listener is backwards compatible, broadcaster is not. See ticket `#2381 <https://github.com/ros/geometry/issues/2381>`_
* migration part 1
