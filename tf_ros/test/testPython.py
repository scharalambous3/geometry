...
class TestPython(unittest.TestCase):
    ...
    def common(self, t):
        ...
        m.transform.rotation = geometry_msgs.msg.Quaternion(*tf_ros.transformations.quaternion_from_euler(0, 0, 0))
        t.setTransform(m)
        ...
        self.assertRaises(tf_ros.Exception, lambda: t.getLatestCommonTime("MANDALAY", "JUPITER"))
        self.assertRaises(tf_ros.LookupException, lambda: t.lookupTransform("MANDALAY", "JUPITER", rospy.Time()))

        # Ask for transform for valid frames, but more than 10 seconds in the past.  Should raise ExtrapolationException
        self.assertRaises(tf_ros.ExtrapolationException, lambda: t.lookupTransform("THISFRAME", "PARENT", rospy.Time(2)))

        #### print t.lookupVelocity("THISFRAME", "PARENT", rospy.Time(15), rospy.Duration(5))

    def test_smoke(self):
        t = tf_ros.Transformer()
        self.common(t)
    ...
    def test_wait_for_transform(self):
        ...
        self.assertRaises(tf_ros.Exception, lambda: t.waitForTransform("PARENT", "THISFRAME", rospy.Time(), timeout))
        # Check for dedicated thread exception, non-existing frames
        self.assertRaises(tf_ros.Exception, lambda: t.waitForTransform("MANDALAY", "JUPITER", rospy.Time(), timeout))
        ...
        start = process_time()
        self.assertRaises(tf_ros.Exception, lambda: t.waitForTransform("PARENT", "THISFRAME", rospy.Time(25), timeout))
    ...
    def test_cache_time(self):
        ...
        for cache_time in range(2, 98):
            t = tf_ros.Transformer(True, rospy.Duration(cache_time))
            m = geometry_msgs.msg.TransformStamped()
            m.header.frame_id = "PARENT"
            m.child_frame_id = "THISFRAME"
            m.transform.translation.y = 5.0
            ...
            for ti in range(1, 100 - cache_time):
                self.assertRaises(tf_ros.ExtrapolationException, lambda: t.lookupTransform("THISFRAME", "PARENT", rospy.Time(ti)))
            for ti in range(100 - cache_time, 100):
                t.lookupTransform("THISFRAME", "PARENT", rospy.Time(ti))

    def test_subclass(self):
        class TransformerSubclass(tf_ros.Transformer):
            def extra(self):
              return 77
...
if __name__ == '__main__':
    rostest.unitrun('tf_ros', 'directed', TestPython)
